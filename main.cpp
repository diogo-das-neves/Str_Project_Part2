...
volatile bool alarm; // #TODO replace this with alarm_clock | temp_alarm, since
                     // they are separate
...
void vTask_Temp_Light_Alarm(void *pvParamaters) {
  ...
  for (;;) {
    ...
    if (sensor_read >= (float)high_threshold_TH) {
      ...
      float H =
          (1.0 - (sensor_read - (float)low_threshold_TL) /
                     ((float)high_threshold_TH - (float)low_threshold_TL)) *
          240.0;
...
int main(void) {
  ...
  SensorTimer =
      xTimerCreate("SensorTimer", pdMS_TO_TICKS(1000 * monitoring_period_PMON),
                   pdTRUE, NULL, TempTimerCallback);

  xTaskCreate(vTask_Serial, "SerialComms Task", 2 * configMINIMAL_STACK_SIZE,
              NULL, 5, NULL);

  xTaskCreate(vTask_Alarm, "Alarm Task", 2 * configMINIMAL_STACK_SIZE, NULL, 2,
              &xTask_Alarm);

  xTaskCreate(vTask_temp, "Temp Task", 2 * configMINIMAL_STACK_SIZE, NULL, 1,
              &xTask_temp);

  xTaskCreate(vTask_LCD, "LCD Task", 2 * configMINIMAL_STACK_SIZE, NULL, 2,
              NULL);

  xTaskCreate(vTask_Temp_Light_Alarm, "TempAlarm Task",
              2 * configMINIMAL_STACK_SIZE, NULL, 2, NULL);

  xTaskCreate(vTask_records, "TempRecords Task", 2 * configMINIMAL_STACK_SIZE,
              NULL, 2, NULL);
